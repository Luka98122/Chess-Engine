...
- [x] `GeneratePawnMoves(Board b, int color)`: Shift operations (`<< 8`, `>> 8`) for single pushes, double pushes, and captures.
- [ ] Add en passant to aforementioned function.
- [x] `GenerateSlidingAttacks(int square, ulong occupiedSquares, int pieceType)`: Attack rays for Bishops, Rooks, and Queens (Magic Bitboards for this).
