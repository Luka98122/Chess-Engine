...
namespace ChessEngine
{
    public static class Bot
    {
        ...
        public static Move Think(Board b, int depth, int topX)
        {
            Span<Move> moves = stackalloc Move[218];
            int moveCount = allMoves.GenerateAllLegalMoves(b, moves, b.SideToMove);
            ...
            for (int i = 0; i < moveCount; i++)
            {
                ...
                if (cache.ContainsKey((b, depth - 1, -beta, -alpha, topX, b.SideToMove)))
                {
                    score = cache[(b, depth - 1, -beta, -alpha, topX, b.SideToMove)];
                }
                else
                {
                    score = -Search(b, depth - 1, -beta, -alpha, topX);
                    cache[(b, depth - 1, -beta, -alpha, topX, b.SideToMove)] = score;
                }
                b.UnmakeMove();
        ...
        private static int Search(Board b, int depth, int alpha, int beta, int topX)
        {
            if (depth == 0)
            {
            ...
            for (int i = 0; i < Math.Min(moveCount, topX); i++) //topx
            {
                b.MakeMove(moves[i]);
                int score = -Search(b, depth - 1, -beta, -alpha, topX);
                b.UnmakeMove();
